...
CREATE DATABASE IF NOT EXISTS `ibge` /*!40100 DEFAULT CHARACTER SET utf8 */;
USE `ibge`;

-- Copiando estrutura para tabela ibge.cidade
CREATE TABLE IF NOT EXISTS `cidade` (
  `cidade_ibge_id` int(11) NOT NULL,
  `cidade_estado_ibge_id` int(11) NOT NULL,
  `cidade_nome` varchar(60) DEFAULT NULL,
  PRIMARY KEY (`cidade_ibge_id`),
  UNIQUE KEY `cidade_ibge_id_UNIQUE` (`cidade_ibge_id`),
  KEY `fk_sys_cidade_sys_estado1_idx` (`cidade_estado_ibge_id`),
  CONSTRAINT `fk_sys_cidade_sys_estado1` FOREIGN KEY (`cidade_estado_ibge_id`) REFERENCES `estado` (`estado_ibge_id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- Copiando dados para a tabela ibge.cidade: ~5.565 rows (aproximadamente)
/*!40000 ALTER TABLE `cidade` DISABLE KEYS */;
INSERT IGNORE INTO `cidade` (`cidade_ibge_id`, `cidade_estado_ibge_id`, `cidade_nome`) VALUES
...
-- Copiando estrutura para tabela ibge.estado
CREATE TABLE IF NOT EXISTS `estado` (
  `estado_ibge_id` int(11) NOT NULL,
  `estado_nome` varchar(60) DEFAULT NULL,
  `estado_uf` enum('AC','AL','AM','AP','BA','CE','DF','ES','GO','MA','MG','MS','MT','PA','PB','PE','PI','PR','RJ','RN','RO','RR','RS','SC','SE','SP','TO') NOT NULL,
  PRIMARY KEY (`estado_ibge_id`),
  UNIQUE KEY `estado_ibge_id_UNIQUE` (`estado_ibge_id`),
  UNIQUE KEY `estado_uf_UNIQUE` (`estado_uf`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- Copiando dados para a tabela ibge.estado: ~27 rows (aproximadamente)
